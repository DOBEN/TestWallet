...
export class WalletConnectProvider extends WalletProvider {
    ...
    async signMessage(
        ...
        try {
            const result = await this.client.request<{ accountTransactionSignatures: AccountTransactionSignature }>({
                topic: this.topic,
                request: {
            ...
            return result.accountTransactionSignatures;
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
        } catch (e: any) {
            if (isWalletConnectError(e)) {
                throw new Error('Signing request rejected in wallet' + e);
            }
            throw e;
        }
    }
